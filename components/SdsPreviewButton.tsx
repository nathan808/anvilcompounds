"use client";

import { useState } from "react";

interface SdsPreviewButtonProps {
  productName: string;
  fileUrl?: string | null;
}

const buttonClass =
  "w-full inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-mock-line hover:border-mock-cobalt/30 text-mock-sub hover:text-mock-navy font-display font-700 text-sm transition-all duration-200 bg-mock-surface2 hover:bg-mock-line/60";

const sdsIcon = (
  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h3.75M9 15h3.75M9 18h3.75M3.75 6.75h.007v.008H3.75V6.75zm0 5.25h.007v.008H3.75V12zm0 5.25h.007v.008H3.75v-.008zM3.75 5.25h16.5v13.5a1.5 1.5 0 01-1.5 1.5H5.25a1.5 1.5 0 01-1.5-1.5V5.25z" />
  </svg>
);

export default function SdsPreviewButton({ productName, fileUrl }: SdsPreviewButtonProps) {
  const [open, setOpen] = useState(false);

  // Real SDS PDF exists for this product — link straight to it.
  if (fileUrl) {
    return (
      <a href={fileUrl} target="_blank" rel="noopener noreferrer" className={buttonClass}>
        {sdsIcon}
        View Safety Data
      </a>
    );
  }

  // No PDF on file yet for this product — keep the descriptive fallback so
  // the button never links to a broken/missing file.
  return (
    <>
      <button onClick={() => setOpen(true)} className={buttonClass}>
        {sdsIcon}
        View Safety Data
      </button>

      {open && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-4"
          onClick={() => setOpen(false)}
        >
          <div
            className="bg-white border border-mock-line rounded-2xl w-full max-w-md overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-6 py-5 border-b border-mock-line flex items-center justify-between">
              <div>
                <p className="font-mono text-[10px] text-mock-sub tracking-[0.2em] uppercase mb-1">
                  Safety Data Sheet
                </p>
                <h3 className="font-display font-700 text-mock-navy text-lg">{productName}</h3>
              </div>
              <button
                onClick={() => setOpen(false)}
                className="w-8 h-8 rounded-lg bg-mock-surface2 hover:bg-mock-line/60 border border-mock-line flex items-center justify-center text-mock-sub hover:text-mock-navy transition-all shrink-0"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>

            <div className="px-6 py-5 space-y-4">
              <p className="font-body text-sm text-mock-sub leading-relaxed">
                A Safety Data Sheet (SDS) documents handling, storage, and hazard information for
                this compound. A full SDS and a technical/research reference guide are included
                with every purchase.
              </p>

              <button
                onClick={() => setOpen(false)}
                className="w-full text-center py-3 bg-mock-cobalt hover:bg-mock-cobaltInk text-white font-display font-700 text-sm rounded-xl transition-all duration-300"
              >
                Close
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
